...
class CartService:
    ...
    def add_item(self, user_id: str, payload: CartItemAdd) -> CartResponse:
        ...
        if not existing:
            if self.cart_repo.count_items(cart_id) >= MAX_CART_ITEMS:
                raise ValidationError(
                    f"Cart is full — max {MAX_CART_ITEMS} products allowed"
                )
            ...
            if new_qty > MAX_ITEM_QUANTITY:
                raise ValidationError("Quantity limit exceeded")
        ...
        logger.info("cart_item_added", user_id=user_id, product_id=product_id)
        return self._build_response(user_id)
